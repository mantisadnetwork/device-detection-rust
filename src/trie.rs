...
pub struct TrieDeviceMatch<'detector> {
    ...
    dataset: *mut fiftyoneDegrees_dataset_t,
...
impl TrieDeviceDetector {
    pub fn new(file_location: &str, properties: Vec<PropertyName>) -> Result<TrieDeviceDetector, u32> {
        let provider = unsafe {
        ...
        if status == e_fiftyoneDegreesDataSetInitStatus_DATA_SET_INIT_STATUS_SUCCESS {
